...
function registrar_leitura(obstaculo) {
    ...
    banco.conectar().then(() => {
        ...
        INSERT into abastecimentos (nivelSensor,dataAbastecimento,fkFornecedora,fkFileira)
        values ('${obstaculo}',CONVERT(Datetime, '${agora()}', 120),1,1);
        
        delete from abastecimentos where idAbastecimento not in 
        (select top ${registros_mantidos_tabela_leitura} idAbastecimento from abastecimentos order by idAbastecimento desc);`);
...
if (gerar_dados_aleatorios) {
	...
	setInterval(function() {
		...
		registrar_leitura(parseInt(Math.random() * 3)+1)
